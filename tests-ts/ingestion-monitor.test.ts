...
describe("checkIngestionHealth", () => {
  it("returns healthy when latest success is within threshold", () => {
    const runs = [
      makeRun({
        status: "success",
        finished_at: new Date(Date.now() - 60 * 60_000).toISOString(),
      }),
    ];
...
describe("formatIngestionAlert", () => {
  it("produces ai-todo note with title and tags", () => {
    const result = checkIngestionHealth([], 4);
    const payload = formatIngestionAlert(result);
    expect(payload.title).toContain("[告警]");
    expect(payload.title).toContain("未成功运行");
    expect(payload.tags).toEqual(["monitoring", "ingestion_alert"]);
  });
  ...
  it("includes last success info when available", () => {
    ...
    const result = checkIngestionHealth(runs, 4);
    const payload = formatIngestionAlert(result);
    expect(payload.title).toContain("上次成功:");
    expect(payload.title).not.toContain("无记录");
  });

  it("handles no success records gracefully", () => {
    const result = checkIngestionHealth([], 4);
    const payload = formatIngestionAlert(result);
    expect(payload.title).toContain("无记录");
  });
});
...
vi.mock("@/lib/integrations/ai-todo-client", () => ({
  AiTodoClient: vi.fn().mockImplementation(() => ({
    createNote: vi.fn().mockResolvedValue(undefined),
  })),
  AiTodoClientError: class extends Error {},
}));
...
describe("GET /api/v1/ingestion/monitor", () => {
  afterEach(() => {
    ...
    delete process.env.AI_TODO_API_URL;
    delete process.env.AI_TODO_SPACE_ID;
    delete process.env.AI_TODO_SPACE_TOKEN;
  ...
  it("sends ai-todo alert when unhealthy and env vars are set", async () => {
    process.env.CRON_SECRET = "test-secret";
    process.env.AI_TODO_API_URL = "https://ai-todo.stringzhao.life";
    process.env.AI_TODO_SPACE_ID = "test-space-id";
    process.env.AI_TODO_SPACE_TOKEN = "ait_test-token";
    vi.mocked(listRecentIngestionRuns).mockResolvedValue([]);
    ...
    expect(AiTodoClient).toHaveBeenCalledWith(
  ...
  it("does not send alert when alert=false", async () => {
    ...
    expect(response.status).toBe(503);
    expect(body.alert_sent).toBe(false);
    expect(AiTodoClient).not.toHaveBeenCalled();
  });

  it("does not send alert when AI_TODO env vars are missing", async () => {
    process.env.CRON_SECRET = "test-secret";
    vi.mocked(listRecentIngestionRuns).mockResolvedValue([]);
  ...
  it("records alert_error when ai-todo send fails", async () => {
    process.env.CRON_SECRET = "test-secret";
    process.env.AI_TODO_API_URL = "https://ai-todo.stringzhao.life";
    process.env.AI_TODO_SPACE_ID = "test-space-id";
    process.env.AI_TODO_SPACE_TOKEN = "ait_test-token";
    vi.mocked(listRecentIngestionRuns).mockResolvedValue([]);
    vi.mocked(AiTodoClient).mockImplementation(
      () =>
        ({
          createNote: vi.fn().mockRejectedValue(new Error("ai-todo down")),
        }) as unknown as AiTodoClient,
    );
    ...
    expect(response.status).toBe(503);
    expect(body.alert_sent).toBe(false);
    expect(body.alert_error).toBe("ai-todo down");
  });
});
