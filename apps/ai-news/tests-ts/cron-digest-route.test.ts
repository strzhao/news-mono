...
import {
  analysisArchiveEnabled,
  buildDigestArgv,
} from "@/lib/routes/cron-digest-helpers";
...
describe("cron digest route helpers", () => {
  ...
  it("buildDigestArgv supports query params", () => {
    const argv = buildDigestArgv(
      new URL(
        "https://example.com/api/cron_digest?date=2026-02-28&ignore_repeat_limit=1&top_n=12",
      ),
      "Asia/Shanghai",
      "/tmp/reports",
    );
  ...
  it("analysisArchiveEnabled is false by default", () => {
    ...
    expect(
      analysisArchiveEnabled(new URL("https://example.com/api/cron_digest")),
    ).toBe(false);
  });
  ...
  it("analysisArchiveEnabled supports query override", () => {
    expect(
      analysisArchiveEnabled(
        new URL("https://example.com/api/cron_digest?archive_analysis=1"),
      ),
    ).toBe(true);
  });
});
