...
export class ConsumptionClient {
  ...
  private parseSourceDailyPayload(
    payload: unknown,
  ): Record<string, Record<string, number>> {
    if (!payload || typeof payload !== "object") {
      throw new ConsumptionClientError(
        "Source stats payload must be an object",
      );
    }
    const rows = (payload as Record<string, unknown>).rows;
    if (!Array.isArray(rows)) {
      throw new ConsumptionClientError(
        "Source stats payload.rows must be a list",
      );
    }
    ...
    for (const row of rows) {
      ...
      const sourceId = String(
        (row as Record<string, unknown>).source_id || "",
      ).trim();
  ...
  private parseTypeDailyPayload(
    ...
    if (!Array.isArray(rows)) {
      throw new ConsumptionClientError(
        "Type stats payload.rows must be a list",
      );
    }
    ...
    for (const row of rows) {
      ...
      const primaryType = String(
        (row as Record<string, unknown>).primary_type || "",
      ).trim();
      const date = String((row as Record<string, unknown>).date || "").trim();
      const rawClicks = Number((row as Record<string, unknown>).clicks || 0);
      ...
      typeDaily[primaryType][date] =
        (typeDaily[primaryType][date] || 0) + clicks;
    }
    return typeDaily;
  }
}
